...
- compare both frontier lanes on the same focused probes:
...
- probes `openai-codex` by default
...
- comparing the OAuth OpenAI Codex lane and the Claude CLI lane under CopeNet's harness
...
3. `uv run python -m benchmarks.coding.run --dry-run`, then a live subset when harness behavior changed
