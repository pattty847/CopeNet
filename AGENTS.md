...
- **Three levels in the thread, not two.** Group header → one titled row per action (`toolRowTitle`: "Ran shell command: …", "Read …/market/model_tables.py") → the row expands **in place**. The preview blocks each own a bounded scroll box; do not wrap them in a second one. The model titles each CALL with a required `activity_title` (stripped before the handler runs, carried as `activityTitle` on the live call/result events and the persisted parts), so the title belongs on that call's row; the hover keeps what was literally called. A closed group and the settled trail box cover many calls, so their headers stay derived counts ("Ran 2 commands, read model_tables.py") — one call's title would mislabel the rest. A group still running shows its latest title, which is how a collapsed group is followed live. A call with no title (direct calls, older transcripts) derives its row from tool id + target, and a tool without a phrase shows its result summary. The live handlers in `wsChatEvents.ts` build parts by hand: a new part field must be added there as well as in `normalizeMessageParts`, or it only appears after a reload.
...
- **Deep detail lives in the overlay.** The in-place expansion is a bounded preview; the whole body, the arguments, the policy decision and the per-turn internals open `InspectorDrawer` (a portal overlay: 680px, own scroll, Escape) via `setInspectorTarget`. Never render an unbounded tool body between two chat messages, and for a new kind of detail add an `InspectorTarget` kind rather than a new inline panel.
- **One derivation renders run internals everywhere.** `runtime/runInternals.ts` turns a `SessionRunRecord` plus its lifecycle trace into the stat line and the what-it-saw / what-it-did / how-it-worked / why-it-stopped / raw-trace sections; `components/runtime/RunInternals.tsx` renders it in the drawer (`showDid={false}` — the thread already lists the calls) and in the Observability inspector (`showDid` default), with `internalsPalette.ts` selecting `operator-*` vs `shell-*` classes. Add to the derivation, not to a per-surface renderer. Three rules that are easy to break: a chat-only turn must keep its "Context it saw" row (that is the `promptedToolUse: false` case, where the question is asked most); the "Context it saw" row closes the settled message rather than sitting inside the trail box, because it is a fact about the whole turn; and `inputTokenEstimate` counts **messages only** — never label it as everything the model saw.
...
- **Do not update README screenshots as a side effect of ordinary UI work.** The README has one fixed six-image product overview: the book-fair banner, Home, Agents, Market, the ticker workspace, and Observability. Add, replace, or recapture those images only when the operator explicitly requests a documentation or screenshot refresh. Put detailed feature descriptions in `docs/FEATURE-GLOSSARY.md`; do not grow a screenshot-per-feature tour. When a requested capture is necessary, prefer Browser Use for localhost verification and capture, with Playwright as the fallback.
- **Before committing any documentation screenshot, check the frame for operator-specific data** — the Market Monitor "since you last looked" strip and daily-briefing header render a real portfolio dollar figure (`Book`/`Portfolio $…`), and the Agents Console Inspector's Destinations panel can show a real phone number or other personal identifier. Either crop those elements out, capture a clean state (empty portfolio / no destinations configured), or skip that panel and note in the PR/commit body which section still needs a sanitized screenshot. This is a live case, not hypothetical: a full pass was needed on 2026-07-31 (commit `fca5acb`) to strip already-committed screenshots that leaked exactly this data. See the operator-data rule under Version Control & Commit Discipline below — it applies to screenshots the same as it does to fixtures and logs.
...
- **Every run carries its own coding metrics.** `core/harness/coding_metrics.py` is the one rule set for the
  questions the harness audit used to ask of traces by hand — redundant reads, reads after the agent's own
  edit, searches over the 200-match cap, edits refused as stale, exact repeats, blind retries, verification
  commands and whether one ran after the last edit, recovery after a red run. Run finalization applies it
  to the run's tool steps and stamps the compact block on the run record as `codingMetrics` (None for a
  chat-only turn); `runInternals.ts` renders it as the "How it worked" section and as verdicts. The
  benchmark analyzer (`benchmarks/coding/trace_analysis.py`) builds its richer per-trace output on the
  same `analyze_tool_calls`, so a benchmark number and a real-session number mean the same thing. Extend
  the rules there, never in a second place.
...
- **Before pushing to the public remote:** sweep for secret-shaped strings and run at least a syntax/compile smoke check.
- **Feature work happens on a branch; the agent manages the branch, the operator decides the merge.** Start a `<agent>/<feature>` branch before the first edit of anything beyond a docs fix or a one-line repair, and keep working there across the operator's follow-up requests while they belong to the same feature. Commit logical units on the branch as they land without asking. Do not ask "should we merge?" after each step — iteration is the normal state of a branch.
- **Raise closing the branch once, when the work reads as finished:** the feature does what was asked, the operator has seen it and reacted well, the full test suites pass (not only the focused ones — a focused run hid six broken tests on 2026-09-17), and the next request would naturally be a different feature. Say it in one line with the state ("branch is green, 3 commits, ready to merge to main — want me to?") and carry on if the answer is not yet. If the operator starts an unrelated feature while a branch is open, say so and offer to close the first one before starting a new branch, rather than stacking both on one.
- **Merging and pushing are the operator's call.** Merge to `main` when told to; never push without being asked. Leave the operator's own uncommitted work (docs refreshes, screenshots, another collaborator's edits) out of the feature commit unless told to include it, and say what was left out.
...
- `uv run copenet`
- **A loopback host never schedules market scans.** `sentinel_enabled()` defaults off when
  `COPNET_HOST` is unset or loopback and on for `tailscale` / explicit binds; `COPNET_MARKET_SENTINEL=0/1`
  overrides either way. So a localhost checkout left running beside the tailnet host cannot
  fire the 09:45 sweep. Notification delivery still runs on every host.
