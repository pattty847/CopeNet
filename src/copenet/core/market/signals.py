...
def compute_rrg_tail(symbol: str, name: str, frame: pd.DataFrame, benchmark: pd.DataFrame, *, points: int = 8) -> RrgSector:
    asset = _normalized_frame(frame)
    bench = _normalized_frame(benchmark)
    if asset.empty or bench.empty:
        return RrgSector(symbol=symbol, name=name, tail=[], quadrant="lagging")
    joined = pd.concat(
    ...
    if len(joined) < 8:
        return RrgSector(symbol=symbol, name=name, tail=[], quadrant="lagging")
    rs = joined["asset"] / joined["bench"]
    center = rs.rolling(10, min_periods=4).mean()
    ratio = ((rs / center) - 1).fillna(0) * 100
    momentum = _zscore(ratio.diff(4).fillna(0)) * 3
    tail = [
        {"x": round(float(x), 2), "y": round(float(y), 2)}
        for x, y in zip(ratio.tail(points).tolist(), momentum.tail(points).tolist(), strict=False)
    ...
    if last["x"] >= 0 and last["y"] >= 0:
        ...
        quadrant = "improving"
    return RrgSector(symbol=symbol, name=name, tail=tail, quadrant=quadrant)  # type: ignore[arg-type]
...
def _zscore(series: pd.Series) -> pd.Series:
    rolling = series.rolling(10, min_periods=4)
    mean = rolling.mean()
