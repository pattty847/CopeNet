...
def chart_system_overlay(context: MarketTurnContext | None) -> str:
    ...
    return (
        ...
        "complete chart packet and opens with a guide to its own layout: a header, a Drawings table "
        "of everything painted on the chart (the operator's lines with their slope per candle and "
        "value at the latest candle; resolve 'the red line' against it and reason from its numbers "
        "rather than re-deriving them), and one exact time-series table (candles plus same-timeframe "
        "indicator and anchored-VWAP columns) for the delivered coverage. Cite those rows "
