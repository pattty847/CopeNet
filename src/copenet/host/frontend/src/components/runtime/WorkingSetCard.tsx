...
function InlineStrip({ icon: Icon, tone, children }: {
  ...
  const toneCls =
    tone === 'accent'
      ...
      : tone === 'error'
      ? 'text-operator-error'
      : 'text-operator-muted/85';
  return (
    <div className={`mx-3 mt-2 flex items-center gap-2 border-b border-operator-border/55 px-2 py-1.5 ${toneCls}`}>
...
export function WorkingSetCard({ sessionKey, isDraft }: WorkingSetCardProps) {
  ...
  if (resource.status === 'loading') {
    return (
      <div className="mx-3 mt-2 border-b border-operator-border/55">
        <div className="px-2 py-1.5 flex items-center gap-2">
          <div className="shimmer h-3 w-3 rounded-sm bg-operator-panel" />
          <div className="shimmer h-2 w-16 rounded bg-operator-panel" />
          <div className="shimmer h-2 w-full max-w-xs rounded bg-operator-panel" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`${isMobile ? 'mx-3 mt-2' : 'mx-3 mt-2'} border-b border-operator-border/55 overflow-hidden`}>
      <button
        onClick={() => setExpanded((v) => !v)}
        className={`w-full flex items-center gap-2 text-left hover:bg-operator-panel/40 transition-colors duration-150 ${
          isMobile ? 'px-2 py-1.5' : 'px-2 py-1.5'
        }`}
      >
        <Brain className="h-3.5 w-3.5 shrink-0 text-operator-accent/85" />
        <span className={`flex items-center gap-1 text-[9.5px] font-semibold uppercase tracking-[0.14em] ${status.text}`}>
          <span className="relative flex h-1.5 w-1.5">
            <span className={`pulse-live absolute inline-flex h-full w-full rounded-full ${status.dot} opacity-55`} />
            <span className={`relative inline-flex h-1.5 w-1.5 rounded-full ${status.dot}`} />
          </span>
          {status.label}
        </span>
        <span className={`min-w-0 flex-1 truncate text-[12px] text-operator-text/85 leading-snug ${expanded ? 'whitespace-normal line-clamp-2' : ''}`}>
          {workingSet.taskSummary}
        </span>
        <span className="hidden text-[10px] text-operator-muted/55 font-mono tabular-nums sm:inline shrink-0">{freshness(workingSet.updatedAt)}</span>
        {expanded ? (
          <ChevronUp className="h-3.5 w-3.5 shrink-0 text-operator-muted/65" />
        ) : (
          <ChevronDown className="h-3.5 w-3.5 shrink-0 text-operator-muted/65" />
        )}
      </button>
