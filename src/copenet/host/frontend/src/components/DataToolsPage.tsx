...
function MediaImportsPage() {
  ...
  async function discussAsset(detail: MediaAssetDetail) {
    ...
    try {
      const attachment = await createMediaChatAttachment(detail.assetId);
      setDraftComposerAttachmentSeed(attachment);
      setDraftOpen(true);
      setCurrentSection('agents');
    } catch (error) {
      setDiscussError(error instanceof Error ? error.message : 'Could not attach this transcript.');
    } finally {
      setDiscussing(false);
    }
  }
