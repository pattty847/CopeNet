...
def create_app_router(
    ...
    from copenet.host.app_api_media import register_media_routes
    ...
    async def upload_chat_attachment(
        ...
        """Persist one composer image or text attachment; returns its id + metadata.

        The chat send then references the id via `attachmentIds`, and the
        orchestrator inlines it into the model input.
        """
        try:
    ...
    register_media_routes(
        ...
        require_media_access=require_media_access,
    )
    return router
