...
from .rpc_market_intraday import handle_market_intraday_get, handle_market_intraday_intervals, handle_market_ticker_profile_get
...
from .rpc_market_forecasts import MARKET_FORECAST_HANDLERS
...
async def _route_rpc(req, send_json: SendJson, orchestrator, tasks: set, broadcast: SendJson, quote_subscription=None) -> None:
    ...
    elif req.method == "market.ticker.profile.get":
        await handle_market_ticker_profile_get(req.id, req.params, send_json, orchestrator)
    ...
    elif req.method in MARKET_FORECAST_HANDLERS:
        await MARKET_FORECAST_HANDLERS[req.method](req.id, req.params, send_json, orchestrator,
            **({"broadcast": broadcast} if req.method == "market.forecast.request" else {}))
